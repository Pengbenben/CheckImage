...
;(function(window, document) {
    var easyUploader = function(configs) {
        if (!configs || !configs.id) { alert('Missing configuration parameters'); return; }
    ...
    easyUploader.prototype = {
        ...
        initPlugin: function (configs) {
            ...
            if (!container) {
                alert("No ID was found" + this.configs.id + "Render container for");
                return;
            } else {
            ...
            $(".btn-upload-file").off("click").on("click", function () {
                ...
                if (hasLoading) {
                    if (that.ajax.isReady) {
                        var message = 'Uploading, please wait';
                    ...
                    if (that.files.length < that.configs.maxCount && index < that.fileObj.fileList.length) {
                        ...
                        if (that.configs.multiple && that.fileObj.fileList.length > 1) {
                            ...
                            if (countDiff > 0 && isSizeOver) {
                                message = 'The number of files exceeds, more than' + countDiff + '；The size of some files exceeds. The allowed size is' + that.configs.maxSize + 'M';
                                onAlert(message);
                            } else if (countDiff > 0 && !isSizeOver) {
                                message = 'The number of files exceeds, more than' + countDiff;
                                onAlert(message);
                            } else if (countDiff <= 0 && isSizeOver) {
                                message = 'The size of some files exceeds. The allowed size is' + that.configs.maxSize + 'M';
                                onAlert(message);
                            }
                        } else {
                            if (countDiff > 0 && fileSize > that.configs.maxSize) {
                                message = 'The number of pieces exceeds, the file size exceeds; the current file is' + fileSize + 'M，Allowed size is' + that.configs.maxSize + 'M';
                                onAlert(message);
                            } else if (countDiff > 0 && fileSize <= that.configs.maxSize) {
                                message = 'The number of files exceeds';
                                onAlert(message);
                            } else if (countDiff <= 0 && fileSize > that.configs.maxSize) {
                                message = 'File size exceeds, current file is ' + fileSize + 'M，Allowed size is' + that.configs.maxSize + 'M';
                                onAlert(message);
                            }
                        }
                        $(that.node.input).val("");
